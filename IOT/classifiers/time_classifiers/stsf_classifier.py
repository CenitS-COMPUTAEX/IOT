...
class StsfClassifier(BaseTimeClassifier):
	"""
	Supervised Time Series Forest model.
	Implemented using the Aeon library because SkTime doesn't support multivariate TSF.
	"""

	trained_model: SupervisedTimeSeriesForest | None

	def __init__(self, logger: Logger, model_dump: object = None):
		"""
		Creates an instance of this model.
		model_dump: Trained STSF model. If unspecified, this instance must be trained before it can be used for prediction.
		"""
		super().__init__(logger)
		self.trained_model = model_dump

	def supports_missing_values(self) -> bool:
		return SupervisedTimeSeriesForest.get_class_tag("capability:missing_values")

	def get_classifier_type(self) -> TimeClassifierType:
		return TimeClassifierType.STSF
	...
	def _train_model(self, data: TimeDataset):
		...
		model = SupervisedTimeSeriesForest(n_estimators=200, n_jobs=-1, random_state=0, use_pyfftw=False,
