...
class FreshprinceClassifier(BaseTimeClassifier):
	"""
	Combines TSFresh and a Rotation Forest classifier.
	"""

	trained_model: FreshPRINCEClassifier | None
	...
	def supports_missing_values(self) -> bool:
		return FreshPRINCEClassifier.get_class_tag("capability:missing_values")

	def get_classifier_type(self) -> TimeClassifierType:
		return TimeClassifierType.FreshPRINCE
	...
	def _train_model(self, data: TimeDataset):
		...
		model = FreshPRINCEClassifier(n_jobs=-1, random_state=0, default_fc_parameters="minimal")
