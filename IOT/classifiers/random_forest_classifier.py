...
class RandomForestClassifier(BaseClassifier):
	...
	Random Forest model
	"""
	trained_model: "RFC | None"

	def __init__(self, logger: Logger, model_dump: object = None):
		"""
		Creates an instance of this model.
		model_dump: Trained RF model. If unspecified, this instance must be trained before it can be used for prediction.
		"""
		super().__init__(logger)
		self.trained_model = model_dump

	def get_classifier_type(self) -> ClassifierType:
		return ClassifierType.RANDOM_FOREST

	def get_model_dump_object(self) -> object:
		return self.trained_model

	def _train_model(self, x, y):
		model = RFC(n_estimators=200, n_jobs=-1, random_state=0)
		model.fit(x, y)
